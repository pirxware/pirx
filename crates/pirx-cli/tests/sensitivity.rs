...
mod tests {
    ...
    fn cli_sobol_runs() {
        let dir = tempfile::tempdir().unwrap();
        let output = dir.path().join("sobol_result.json");
        ...
        let status = Command::new(pirx_bin())
            .args([
                ...
                fixtures_dir().join("cultivation_hw.toml").to_str().unwrap(),
                "--sweep",
                fixtures_dir().join("sweep_sobol.toml").to_str().unwrap(),
                "--output",
                output.to_str().unwrap(),
            ])
            .status()
        ...
        assert_eq!(
            status.code(),
            Some(0),
            "pirx sensitivity sobol should exit 0"
        );
        assert!(output.exists(), "output file must exist");
    }

    #[test]
    fn cli_sobol_output_valid_json() {
        let dir = tempfile::tempdir().unwrap();
        let output = dir.path().join("sobol_result.json");
        ...
        let json = std::fs::read_to_string(&output).unwrap();
        let result: pirx_sensitivity::SobolResult =
            serde_json::from_str(&json).expect("output must be valid SobolResult JSON");
